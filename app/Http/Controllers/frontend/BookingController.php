...
class BookingController extends Controller
{
    ...
    {
        //$date = Carbon::parse('this wednesday')->toDateString();
        $date = Carbon::createFromDate(2021, 3, 7)->toDateString();
        if (strtotime($date) > strtotime(Carbon::parse('today')->toDateString())) {
            //$date = Carbon::parse('this wednesday')->toDateString();
            $date = Carbon::createFromDate(2021, 3, 7)->toDateString();
        }
        $request->session()->put('bookingdate', $date);
        $timeslots = Timeslot::where('hidden', '=', '0')->where('start', '>=', '20:00')->where('start', '<=', '21:40')->orderBy('start', 'asc')
            ->get();
    ...
    public static function cancellation(Request $request)
    {
        ...
        if ($possible == 0) {
            return "We are facing some problems, please try again after some time.";
        }
        $booking = Booking::where('id', '=', $request->route('id'))->where('user_id', '=', Auth::user()->id)->where('cancel', '=', 0)->first();
        $booking->cancel = 1;
        $booking->save();
    ...
    {
        if (!in_array($request->pay, ['0', '1'])) { //try this one for tokens
        ...
        if ($exist == 0) {
            ...
            if ($request->pay == 0) {
                ...
                if (FHelper::wallet() >= 1) {
                    ...
                    return "You do not have sufficient token!";
                }
            }
