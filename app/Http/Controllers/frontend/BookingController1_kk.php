...
class BookingController1_kk extends Controller
{
    ...
    public static function timeslot(Request $request)
    {
        ...
        $date = Carbon::createFromDate(2019, 10, 19)->toDateString();
        if (strtotime($date) > strtotime(Carbon::parse('this friday')->toDateString())) {
            $date = Carbon::parse('this friday')->toDateString();
            $date = Carbon::createFromDate(2019, 10, 19)->toDateString();
        }
        $request->session()->put('bookingdate', $date);
        $timeslots = Timeslot::where('hidden', '=', '0')->where('start', '>=', '22:00')->where('start', '<=', '23:40')->orderBy('start', 'asc')
            ->get();
        return view('frontend.slots1-kk', ['timeslots' => $timeslots]);
    }
