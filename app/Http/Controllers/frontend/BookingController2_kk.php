...
class BookingController2_kk extends Controller
{
    ...
    public static function timeslot(Request $request)
    {
        ...
        $date = Carbon::createFromDate(2019, 10, 4)->toDateString();
        if (strtotime($date) > strtotime(Carbon::parse('this friday')->toDateString())) {
            $date = Carbon::parse('this friday')->toDateString();
            $date = Carbon::createFromDate(2019, 10, 4)->toDateString();
        }
        $request->session()->put('bookingdate', $date);
        $timeslots = Timeslot::where('hidden', '=', '0')->where('start', '>=', '19:00')->where('start', '<=', '20:40')->orderBy('start', 'asc')
            ->get();
        return view('frontend.slots2-kk', ['timeslots' => $timeslots]);
    }
