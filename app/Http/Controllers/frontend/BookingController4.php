...
class BookingController4 extends Controller
{
    public static function make(Request $request)
    {
        $request->session()->put('bookingdate', $request->route('date'));
        return redirect('timeslot4');
    }

    public static function booking1(Request $request)
    {
    ...
    public static function timeslot(Request $request)
    {
        ...
        $date = Carbon::createFromDate(2020, 10, 16)->toDateString();
        if (strtotime($date) > strtotime(Carbon::parse('this friday')->toDateString())) {
            $date = Carbon::parse('this friday')->toDateString();
            $date = Carbon::createFromDate(2020, 10, 16)->toDateString();
        }
        $request->session()->put('bookingdate', $date);
        $timeslots = Timeslot::where('hidden', '=', '0')->where('start', '>=', '21:00')->where('start', '<=', '22:40')->orderBy('start', 'asc')->get();
        // $timeslots = Timeslot::where('hidden','=','0')->where('start','>=','17:00')
        //                                               ->where('start','<=','18:40')
        //                                               ->where('start','!=','18:00')
        //                                               ->orderBy('start','asc')->get(); 
        return view('frontend.slots4', ['timeslots' => $timeslots]);
    }
