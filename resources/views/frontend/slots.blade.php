...
<div class="container content" >
	<div class="row">
		<div class="col-md-12">
			...
			<tr @if($timeslot->block == 1) class="danger" @endif>
				<td>
		 		...
		 		<td>
					...
					@else
					{{-- insert pax no here, counter is for 5v5 = 10. If the game pax no is 14 (7v7), then use -4  --}}
					{{-- @if(FHelper::bookingcount(1,$timeslot->id) == 0)  --}}
					@if(FHelper::bookingcount(1,$timeslot->id) == -4) 
					...
					{{-- <a href="{{url()}}/booking/{{$timeslot->id}}/court/1" style="background-color: green; color: white; border-radius: 12px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);  " class="btn btn-success">Book Now</a> --}}
					...
					<a href="{{url()}}/booking/{{$timeslot->id}}/court/1" style="background-color: green; color: white; border-radius: 12px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);  " class="btn btn-success">Book Now</a>
					@endif
					 - {{10-FHelper::bookingcount(1,$timeslot->id)}}/14
					@endif
					</td>
				<!--      <td>
					@if(strtotime(date('Y-m-d H:i:s', strtotime(Session::get('bookingdate').' '.$timeslot->end))) <= strtotime(\Carbon\Carbon::now()->addMinutes(0)))
					<a href="{{url()}}/booking/{{$timeslot->id}}/court/2" class="btn btn-danger">Closed</a>
					@else
					@if(FHelper::bookingcount(2,$timeslot->id) == 0)
					<a href="{{url()}}/booking/{{$timeslot->id}}/court/2" class="btn btn-danger">Full</a> 
					@elseif($timeslot->block == 1)
					<a href="{{url()}}/booking/{{$timeslot->id}}/court/2" class="btn btn-danger">Reserved</a>
					@elseif($timeslot->id == 57 )
					<a href="" class="btn btn-danger">Closed</a>
					@else
					<a href="{{url()}}/booking/{{$timeslot->id}}/court/2" style="background-color: green; color: white;" class="btn btn-success">Book Now</a>
					@endif
					 {{-- - {{14-FHelper::bookingcount(2,$timeslot->id)}}/14 --}}
					@endif
					</td>     -->
			...
			<h3 class="text-center" style="font-size:60px">Sorry bookings will be open soon. For more updates please follow our Instagram account - @supersquadbn.</h3>
