			...
			<table class="table table-striped"> 
				...
				<tr @if($timeslot->block == 1) class="danger" @endif>
					...
					<td>
						@if(strtotime(date('Y-m-d H:i:s', strtotime(Session::get('bookingdate').' '.$timeslot->end))) <= strtotime(\Carbon\Carbon::now()->addMinutes(0)))
							<a href="{{url()}}/booking3/{{$timeslot->id}}/court/1" class="btn btn-danger" style="border-radius: 12px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);  " >Closed</a>
						@else
						@if(FHelper::bookingcount(1,$timeslot->id) == 0)
							{{-- <a href="{{url()}}/booking3/{{$timeslot->id}}/court/1" class="btn btn-danger" style="border-radius: 12px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);  " >Full</a>  --}}
							<a href="{{url()}}/booking3/{{$timeslot->id}}/court/1" style="background-color: green; color: white; border-radius: 12px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);" class="btn btn-success">Book Now</a>
				{{-- 	@elseif($timeslot->id == 30)
						...
						@else
							<a href="{{url()}}/booking3/{{$timeslot->id}}/court/1" style="background-color: green; color: white; border-radius: 12px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);" class="btn btn-success">Book Now</a>
						@endif
						{{-- - {{18-FHelper::bookingcount1(1,$timeslot->id)}}/14 --}}
						@endif
					</td>   
				</tr>
				@endforeach
